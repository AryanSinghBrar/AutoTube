...
class AutoTube:
    ...
    def scrape_images(self, url, folder):
        try:
            os.mkdir(folder)
    ...
    def create_video(self, website_url, script):
        image_folder = os.path.join(self.base_path, 'images')  # Construct the image folder path
        audio_output = os.path.join(self.base_path, 'output_audio.wav')  # Construct the audio output path

        # Scrape images
        self.scrape_images(website_url, image_folder)

        # Generate text-to-speech audio
        self.text_to_speech(script, audio_output)
        ...
        output_video_path = os.path.join(self.base_path, 'output_video.mp4')  # Construct the output video path
...
if __name__ == "__main__":
    ...
    autoTube = AutoTube(base_path)
    autoTube.create_video(website_url, script)
